...
Enable the storage provider in `/etc/towbar/towbar.yml`, validate and restart Towbar, and confirm it appears under **Manage → Integrations → Backup providers**. Grant the runtime identity access to the buckets, containers, prefixes, and encryption keys used by workload policies.
...
```yaml title=".towbar/resources/cache.resource.yml" highlight={5-15}
id: cache
name: Redis cache
type: redis
server: 192.0.2.10
