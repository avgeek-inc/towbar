...
All examples use your own API host and an bearer API key. Use a full-access key for mutations. Owner-only operations require the creator to remain a workspace owner.
...
Discover declared environments with `POST /sources/discover`, using repository details from `/github/repositories` and a discovery branch. Connect the repository with `POST /sources/connect`, mapping each selected environment to its branch. Initial sync imports configuration without deploying. For later reconciliation, call `POST /sources/{sourceId}/environments/{environmentId}/syncs`, or `/sources/{sourceId}/environments/syncs` for all connected environments. These explicit syncs may trigger eligible auto-deployments. Inspect each queue outcome and read `/sources/{sourceId}/syncs/{syncId}` for completion and validation errors.
