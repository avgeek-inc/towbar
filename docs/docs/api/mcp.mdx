...
MCP requires an HTTPS Towbar installation. It is unavailable in the default local HTTP installation.
...
3. Copy the MCP URL shown in the page. It ends in `/v1/mcp` on your Towbar host.
...
<Tabs>
  <Tab title="Codex">
    Add this to `~/.codex/config.toml`, replacing the Towbar host:

    ```toml
    [mcp_servers.towbar]
    url = "https://YOUR_TOWBAR_HOST/v1/mcp"
    ...
    ```bash
    codex mcp add towbar --url 'https://YOUR_TOWBAR_HOST/v1/mcp' \
  ...
  <Tab title="Claude Code">
    ...
    ```bash
    claude mcp add --transport http towbar 'https://YOUR_TOWBAR_HOST/v1/mcp' \
...
| Connection or certificate error    | Use the Towbar host, `/v1/mcp`, HTTPS, and a publicly trusted certificate.                        |
