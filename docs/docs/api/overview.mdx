...
<Warning>
  The REST API, MCP, and API-key management are available only when Towbar is
  installed at a public HTTPS URL. For security, they are disabled on
  localhost-only installations.
</Warning>
...
| Integrations | GitHub, GitLab, storage, registry, secrets, platform and notifications                              |
