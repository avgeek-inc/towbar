...
| --------------------- | ------------------------------ | -------------------- |
| REST API              | `https://YOUR_API_HOST/v1/api` | API or API & MCP key |
| MCP                   | `https://YOUR_API_HOST/v1/mcp` | MCP or API & MCP key |
| OpenAPI specification | `GET /v1/api/openapi.json`     | API or API & MCP key |
...
<Steps>
  <Step title="Create a key">
    Open **Manage → API & MCP → Create API key**. Name it for its consumer, choose its purpose, permissions, and expiry, then copy it. Towbar only shows the secret once.
  </Step>
...
| `403`  | Wrong key purpose, read-only mutation, or insufficient role | Check purpose, permissions, and workspace role.  |
