...
Towbar exposes infrastructure operations through REST and MCP. Manage sources, apps, resources, servers, deployments, previews, secrets, integration settings, and your current identity using your current workspace permissions.
...
All documented paths are relative to `/v1/api`. Browser-only login, logout, initial setup, GitHub webhooks, and signed worker endpoints are separate protocols. They are not exposed as MCP tools. API key management, notifications, profile editing, password changes, browser-session management, and the GitHub installation handoff are available only in the control plane. `GET /profile` remains available to identify the current user and workspace.
...
| Category     | Sections                                                                              |
| ------------ | ------------------------------------------------------------------------------------- |
| Sources      | Overview, Sync & manifest, Auto-deploy, Inventory, Secrets                            |
...
| Servers      | Overview, Inventory, Capacity, Checks & preparation, Credentials & trust, Maintenance |
| Deployments  | Overview, Progress & logs, Actions, Security scans                                    |
| Previews     | Lifecycle                                                                             |
| Workspace    | Shared secrets, System health, Identity                                               |
