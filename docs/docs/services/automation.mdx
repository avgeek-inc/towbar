...
description: "Opt a Service into automatic deployment, PR previews, notification routing, and scanning."
---
...
Enable previews for an environment in `towbar.yml`, then opt the Service in with a base domain:
...
Use `notifications` to route an Service's events to additional workspace-configured destinations. It takes effect after a successful environment sync without a deployment. `vulnerabilityScanning` controls image scanning, while `jobs` runs scheduled commands from the deployed image. Follow [Notifications](/docs/integrations/notifications), [Vulnerability scanning](/docs/vulnerability-scanning), and [Scheduled jobs](/docs/services/jobs) for their separate configuration and execution rules.
