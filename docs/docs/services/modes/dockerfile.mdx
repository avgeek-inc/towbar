---
title: "Dockerfile"
description: "Build a Service from an existing Dockerfile and control its build context, target, and cache."
---
...
```yaml title=".towbar/services/dockerfile.service.yml"
...
If this Service needs private packages during build, declare a build secret name in the Service manifest and use a BuildKit secret mount in the Dockerfile. Do not put credentials in build arguments or a committed file. A cache is workload-scoped unless an authorized shared `cache.scope` is configured.
