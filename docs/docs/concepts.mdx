...
| Repository       | A connected GitHub repository                                       | Towbar                                               |
...
| Service          | A source build, prebuilt OCI image, or Compose workload             | `.towbar/services/**/*.service.yml`                  |
...
| Preview          | A Service instance associated with a pull request                   | Enable for the target environment and opt in per app |
...
```mermaid
flowchart LR
  GitHub[GitHub repository] --> Repository[Towbar repository]
  Repository --> Production[Production: mapped branch]
  Repository --> Staging[Staging: mapped branch]
  Production --> ProductionApp[Service instance]
  Production --> ProductionDB[Datastore instance]
  Staging --> StagingApp[Service instance]
  Staging --> StagingDB[Datastore instance]
...
**Git describes the workload.** The root file declares environments. Entity files define images or Dockerfiles, health checks, domains, resource limits, required secret keys, and deployment policy. Keep stable IDs when renaming a Service or Datastore so its history stays attached.
