...
| App        | A service built from a Dockerfile                                      | `towbar.yml` and the entity files under `.towbar/`     |
| Resource   | A service run from an image, including PostgreSQL and Redis            | `towbar.yml` and the entity files under `.towbar/`     |
...
**Towbar stores operational settings and secrets.** Server credentials, SSH trust, concurrency, and encrypted secret values belong in the dashboard. Saving a secret does not deploy it.
...
Production follows `source.branch`. Previews run eligible same-repository pull requests against that branch. Each environment has its own secret values and explicit references, and previews do not clone database resources.
