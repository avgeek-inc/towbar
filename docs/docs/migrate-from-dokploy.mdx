...
Configure the Towbar runtime for the Git provider, registry, backup storage, notifications, log forwarding, and any external-secret provider. Platform integrations appear when configured; Notifications and Log forwarding list all supported providers and show which need configuration.
