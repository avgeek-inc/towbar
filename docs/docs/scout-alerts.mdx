...
- **Performance** lets you find a server, app, or resource by name or server IP. Select it to use the same metrics, time ranges, average/peak controls, environment and container filters, and event history as its entity page. Scout installation stays on the server settings page.
- **Alerts** lists configured rules across the workspace. This page is read-only; **View alert** opens the entity’s **Scout Agent → Alerts** page to manage the rule.
...
Select **View Incident** to open its details in a drawer without leaving the alert list. The drawer shows the original trigger condition, entity, status, severity, start and end times, latest or final reading, and when a notification was last queued. Queued notifications do not confirm delivery.

The chart covers the incident start through now, including after recovery, and refreshes every 30 seconds while open. It shows the relevant metric, the rule’s threshold, and a recovery or closure marker. Tooltip timestamps include the time zone. Longer periods use at most 361 points, preserving the highest reading for upper thresholds or the lowest for lower thresholds. Dotted bridges mark gaps between measurements; they are not measured values.
