...
The `towbar` command manages the control plane installed on a Towbar host. It selects the active release, reads `/etc/towbar/towbar.yml`, and supplies the correct Compose project, images, and release metadata for each operation.
...
<CardGroup cols={2}>
  ...
  >
    Find and validate the YAML configuration, then apply a valid change safely.
  </Card>
  <Card
...
| `towbar config path`                   | Print the active configuration path                    | No            | No                    |
...
| `/etc/towbar/towbar.yml`         | Root-owned runtime configuration; expected mode is `600`     |
...
<Info>
  ...
  operations, backups, and server setup are requested from the dashboard, REST
  API, or MCP rather than from this host CLI.
</Info>
...
Avoid running lifecycle commands concurrently. `install`, `upgrade`, `restart`, and `config validate` acquire the Towbar operation lock and stop if another protected operation is active.
