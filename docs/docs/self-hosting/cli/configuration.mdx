...
description: "Edit Towbar's YAML configuration, validate changes, and restart only after every preflight succeeds."
---

Towbar keeps installation and integration configuration in `/etc/towbar/towbar.yml`, owned by `root:root` with mode `600`. The CLI does not provide an editor or print its contents.

## `towbar config path`

Print the active configuration path. The command accepts no arguments, does not require an installed release, and does not read or reveal the file.
...
The command accepts no parameters. It requires root, Docker Engine, Compose v2, an installed release, and the current configuration.
...
Apply a valid YAML change to the current release.

```bash
sudo towbar restart
```

The command accepts no parameters. It acquires the installation lock, validates the configuration, performs API, worker, integration, notification, and Caddy preflights, then recreates changed services and waits for health.
...
<Steps>
  <Step title="Back up the current file">
    Preserve `/etc/towbar/towbar.yml` in restricted storage before changing
    production configuration. The file contains credentials and encryption keys.
  </Step>
  <Step title="Edit the YAML configuration">
...
<Warning>
  Never replace `security.credentialsKey` on an existing installation. Stored
