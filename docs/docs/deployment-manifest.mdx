...
description: "Declare environments in towbar.yml and configure Services and Datastores in separate entity files."
---

Towbar reads `towbar.yml`, then discovers Service, Compose, and Datastore files under
...
[service example](/examples/hello-towbar.service.yaml). Editor schemas are available for
[the root](/schemas/repository.v2.json), [Services](/schemas/app.v2.json),
[Compose](/schemas/compose.v2.json), and [Datastores](/schemas/resource.v2.json).
...
```yaml title=".towbar/services/web.service.yml" highlight={1-4,14-20}
id: web
name: Web
deployment:
  type: dockerfile
...
## Route notifications for one service or datastore

Workspace destinations in **Manage → Notifications** receive events across all
services and datastores. Add `notifications` to an app, Compose workload, or resource
...
shape works in datastore files. Use `messageThreadId` only for a Telegram forum
...
webhook pair. The service or datastore **Monitor → Notifications** page shows the
...
```yaml title=".towbar/services/web.service.yml" highlight={3-7}
id: web
name: Web
autoDeploy:
  inputs:
...
```yaml title=".towbar/services/web.service.yml" highlight={3-10}
id: web
name: Web
secrets:
  build:
...
Services and datastores can share a Docker network. Resource aliases default to the
resource ID; an SSH tunnel binds only to server loopback:

```yaml title=".towbar/datastores/postgres.datastore.yml" highlight={4-8}
id: postgres
name: PostgreSQL
type: postgres
container:
...
```yaml title=".towbar/services/web.service.yml" highlight={3-6}
id: web
name: Web
preview:
  enabled: true
...
```yaml title=".towbar/services/web.service.yml" highlight={3-4,7-8}
id: web
name: Web
tls:
  mode: direct
...
```yaml title=".towbar/services/files.service.yml" highlight={3-6,9-11}
id: files
name: File uploads
rollout:
  type: recreate
...
```yaml title=".towbar/services/files.service.yml" highlight={5}
container:
  port: 3000
  volumes:
    - name: uploads
...
```yaml title=".towbar/services/web.service.yml" highlight={3-11}
id: web
name: Web
jobs:
  - name: daily-report
...
[Scheduled jobs](/docs/services/jobs) for execution and retry behavior.
