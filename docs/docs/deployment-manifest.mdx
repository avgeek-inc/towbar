...
Select one or more runtime-configured log-forwarding providers by name for an app or resource:
...
environment. This selection applies to the workload's container logs. Provider
endpoints and credentials stay in the Towbar runtime, while Towbar adds app,
deployment, environment, and server context to each forwarded record.
`logDrainAttributes` adds non-sensitive context for a selected provider. See
[Log forwarding](/docs/integrations/log-drains) for provider setup and delivery
limits. If a base manifest defines attributes, also set
`logDrainAttributes: {}` when an environment override sets `logDrains: []`.
