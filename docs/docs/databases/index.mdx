...
CPU and memory defaults, backup format, and fresh-target restore workflow.

Databases are managed [Resources](/docs/datastores). Their individual guides below cover the datastore manifest, credential keys, private connectivity, initialization, and recovery for each engine.

The datastore manifest declares **which environment-variable names the database
needs**. Their values stay out of Git and are saved separately for each Towbar
environment under **Datastore → Settings → Secrets**.
...
The generated [datastore JSON Schema](/schemas/resource.v2.json) contains the
...
Every database guide includes a complete datastore file with `secrets.runtime`.
...
beside the datastore definition.
...
Each preset receives one managed volume at its engine-native data path. Services and
datastores on the same server can join a named `container.network`; the datastore
...
Environment overrides may select different servers, CPU and memory limits, networks,
and tunnel ports. Secret names stay common to the datastore, while their values
