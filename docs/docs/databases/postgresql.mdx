---
title: "PostgreSQL"
description: "Declare a PostgreSQL 17 datastore, its initialization variables, networking, persistence, and recovery policy."
icon: "/assets/database-logos/postgresql.webp"
---

<img
  className="towbar-doc-brand-logo"
  src="/assets/database-logos/postgresql.webp"
  alt="PostgreSQL logo"
  aria-hidden="true"
/>
...
Save all three values under **Datastore → Settings → Secrets** for production.
...
A service on the `application` network can connect to `postgres:5432`. Store its
connection URL as an service runtime secret, for example
