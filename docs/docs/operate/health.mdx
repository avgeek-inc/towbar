...
description: "Distinguish control-plane checks from server checks and service or datastore readiness."
---

Health is scoped. **Manage → System health** checks Towbar's API/database, Temporal, and worker/maintenance processes. A server check tests SSH reachability and records host and workload observations. A service or datastore health result describes its runtime at the time of that observation. One green result cannot substitute for the others.
