...
A deployment is one attempt to put an service or datastore into service. Towbar records its trigger, immutable inputs, progress, duration, and final result separately from Repository sync history.

<div className="towbar-doc-screenshot">
  <div className="towbar-product-light">
    <img
      src="/assets/release-v2/deployments-light.jpg"
  ...
  <div className="towbar-product-dark">
    <img
      src="/assets/release-v2/deployments-dark.jpg"
      alt="Deployment history shows each workload's logo or icon, type, environment, and status."
      width="2560"
      height="1440"
      loading="lazy"
  ...
  <p>Find a deployment by workload, environment, status, or trigger.</p>
</div>
...
The **App / Resource** column shows the workload's logo or icon with its type below the name, such as Container image, Redis, or PostgreSQL. Open a row to inspect its recorded inputs and stages.

On mobile, open **Page menu** to access the filters.

## Deploy manually

Open an service or datastore and choose **Deploy**. The server must be prepared and trusted, and required credentials must be configured. Follow the queued operation through its recorded stages; a queued request is not a completed deployment.
...
Services and datastores are manual unless `autoDeploy` is enabled. `autoDeploy: true` treats commits as deployment inputs. Apps can narrow their inputs with repository globs:
