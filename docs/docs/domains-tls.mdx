...
Towbar uses Caddy on the target server to route public HTTP traffic. Domain settings live with the app or resource in the manifest; provider credentials come from the Towbar runtime configuration.
...
Set `integrations.cloudflare.enabled: true`, `accountId`, and `apiToken` in `/etc/towbar/towbar.yml`, then validate and restart Towbar. Each app or resource selects Cloudflare DNS validation in its manifest. A deployment using `tls.mode: cloudflare-dns` is rejected if the runtime integration is missing. Towbar prepares the server's Caddy DNS module when the first such deployment runs.
