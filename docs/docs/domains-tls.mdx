...
Set `integrations.cloudflare.enabled: true`, `accountId`, and `apiToken` in `/etc/towbar/towbar.yml`, then validate and restart Towbar. The Cloudflare integration and the per-server **Cloudflare TLS** toggle appear only after the complete configuration passes startup validation.
...
Use a scoped account token rather than a personal or global API key. `integrations.cloudflare.zoneId` is optional when the token can discover the required zones. The prepared Caddy build must contain the Cloudflare DNS module.
