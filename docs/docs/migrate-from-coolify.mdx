...
- scheduled tasks, webhooks, preview settings, and external dependencies; and
...
| Standalone supported database       | Managed Datastore with the same supported engine and major version                  |
| Other stateful service              | Image Service or Compose project after validating persistence, health, and recovery |
...
Create `towbar.yml` and one entity file per Service or Datastore. Run schema validation in the repository before connecting it, then connect each environment to its branch. Keep automatic deployments disabled during migration.

## 3. Recreate credentials and integrations

Configure GitHub or GitLab, registries, backup storage, notifications, Cloudflare, and external-secret providers in Towbar's runtime configuration. Check platform providers under **Manage → Integrations**, and notification providers under Manage → Notifications.
