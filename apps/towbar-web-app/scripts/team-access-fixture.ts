...
export function createTeamAccessFixture(
  ...
  async function handle(
    ...
    try {
      ...
      if (
        selected.role !== "admin" &&
        /\/v1\/core\/(?:team(?:\/|$)|settings\/private-keys|aws|azure|gcp|log-drains(?!\/usage\/)|system-health|notifications\/|github(?:$|\/configuration|\/install)|.*\/(?:credentials|host-keys|orphans|cloudflare-tls|auto-deploy-control)(?:\/|$))/.test(
          path,
        )
      )
