...
import { scheduleSourceAutomaticDeployments } from "../../../areas/apps/automatic-deployments.js";
...
internalSourceSyncRoutes.post("/:syncId/auto-deploy", async (context) =>
  context.json(
    await scheduleSourceAutomaticDeployments(
      syncId(context.req.param("syncId")),
    ),
  ),
);
