...
DELETE FROM "towbar_managed_secrets"
WHERE "source_id" IS NOT NULL
  AND "app_id" IS NULL
  AND "server_id" IS NULL;--> statement-breakpoint
...
ALTER TABLE "towbar_managed_secrets" ADD CONSTRAINT "towbar_managed_secret_stage" CHECK ((
    ("towbar_managed_secrets"."stage" IN ('build', 'deployment', 'pre_deploy', 'post_deploy') AND ("towbar_managed_secrets"."owner" = 'workspace:' || "towbar_managed_secrets"."workspace_id"::text OR "towbar_managed_secrets"."app_id" IS NOT NULL))
